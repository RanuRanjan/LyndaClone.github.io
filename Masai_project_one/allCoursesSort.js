...
function showPop(e) {
    ...
    if (e.target.textContent.includes('▼'))
        showMenu(e.target.textContent);
    else if (e.target.textContent.includes('Done'))
        sort_tab();
    else return;
}
function showMenu(con) {
    let x = ``, aaa = sortVal[con.split(' ')[0].toLowerCase()];
    if (!aaa) return;
...
let sortVal = {
    ...
    time: ['All', '10 mins', '10 - 30 min', '10 - 30 min', '30 - 50 min', '1+ hours', '25%'],
    software: ['Photoshop (15,804)', 'Illustrator (7,225)', 'Microsoft Excel (6,845)',

        'After Effects (5,944)', 'Office 365 (4,670)', 'Azure (4,662)',

        'Premiere Pro (3,959)', 'AWS(3,835)', 'InDesign (3,705)', 'JavaScript (3,126)', 'CINEMA 4D (2,521)', 'iOS (2,329)', 'Maya (2,041)', 'PHP (1,942)', 'Microsoft Word (1,750)', '35%']
};
